 ...
 * @Description: 描述：文件操作类.
 * @Author：Chengel_HaltuD
 * @Date：2015-5-30 下午3:05:41
 * @version V1.0
 *
 */
public class F {
	...
	public static  String DB_DIR = "db";
	 ...
	 public static Bitmap getBitmapFromSD(File file,int type,int desiredWidth, int desiredHeight){
		 ...
		 try {
			 ...
			 //文件存在
			 if(type == I.CUTIMG){
				bitmap = I.getCutBitmap(file,desiredWidth,desiredHeight);
		 	 }else if(type == I.SCALEIMG){
		 		bitmap = I.getScaleBitmap(file,desiredWidth,desiredHeight);
		 	 }else{
		 		bitmap = I.getBitmap(file);
		 	 }
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static Bitmap getBitmapFromURL(String url,int desiredWidth, int desiredHeight){
		Bitmap bitmap = null;
		try {
			bitmap = I.getBitmap(url,desiredWidth,desiredHeight);
	    } catch (Exception e) {
	    	L.D( "下载图片异常："+e.getMessage());
		}
		return bitmap;
	}
	...
	public static Bitmap getBitmapFromSrc(String src){
		Bitmap bit = null;
		try {
			bit = BitmapFactory.decodeStream(F.class.getResourceAsStream(src));
	    } catch (Exception e) {
	    	L.D( "获取图片异常："+e.getMessage());
		}
		return bit;
	}
