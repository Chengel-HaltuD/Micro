 ...
 * 
 * @ClassName: I
 * @Description: I是AbImageUtil 描述：图片处理类.
 * @Author：Chengel_HaltuD
 * @Date：2015-5-30 下午3:06:13
 * @version V1.0
 *
 */
public class I {
	...
	public static Bitmap getBitmap(InputStream inputStream,int desiredWidth, int desiredHeight) {
		Bitmap bitmap = null;
		try {
			byte [] data = Y.stream2bytes(inputStream);
	...
	public static byte[] view2Bytes(View view,
		...
		byte[] b = null;
		try {
			Bitmap bitmap = I.view2Bitmap(view);
			b = I.bitmap2Bytes(bitmap, compressFormat, true);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return b;
	...
	public static String getHashCode(Bitmap bitmap) {
		...
		for (int i = 0; i < comps.length; i += 4) {
			...
			hashCode.append(Z.binaryToHex(result));
		}
	...
	public static String getDCTHashCode(Bitmap bitmap) {
		...
		int[][] pxMatrix = Z.arrayToMatrix(pixels, width, height);
		double[][] doublePxMatrix = Z.intToDoubleMatrix(pxMatrix);

		// 计算DCT,已经变成8*8了
		double[][] dtc = FDCT.fDctTransform(doublePxMatrix);

		// 计算平均值。
		double[] dctResult = Z.matrixToArray(dtc);
		int avgPixel = Z.average(dctResult);
