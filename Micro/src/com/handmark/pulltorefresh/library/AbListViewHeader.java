...
public class AbListViewHeader extends LinearLayout {
	...
	private void initView(Context context) {
		...
		V.setPadding(headerView, 0, 10, 0, 10);
		
		//显示箭头与进度
		FrameLayout headImage =  new FrameLayout(context);
		arrowImageView = new ImageView(context);
		//从包里获取的箭头图片
		arrowImage = F.getBitmapFromSrc("image/arrow.png");
		...
		V.setPadding(headTextLayout,0, 0, 0, 0);
		...
		V.setTextSize(tipsTextview,30);
		V.setTextSize(headerTimeView,27);
		...
		layoutParamsWW3.rightMargin = V.scaleValue(mContext, 10);
		...
		switch(state){
			case STATE_NORMAL:
				...
				if(lastRefreshTime==null){
					lastRefreshTime = D.getCurrentDate(D.dateFormatHMS);
