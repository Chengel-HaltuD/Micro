...
import com.micro.db.MicroBasicDBDao;
import com.micro.db.orm.MicroTableHelper;
...
import com.micro.db.orm.annotation.Relations;
import com.micro.db.orm.annotation.RelationsType;
...
/**
 * 
 * @ClassName: MicroDBDaoImpl
 * @Description: 描述：数据库表操作类接口实现类
 * @Author：Chengel_HaltuD
 * @Date：2015-5-30 下午2:34:39
 * @version V1.0
 *
 */
public class MicroDBDaoImpl<T> extends MicroBasicDBDao implements MicroDBDao<T> {
	 ...
	 * @param clazz 映射对象实体
	 */
	public MicroDBDaoImpl(SQLiteOpenHelper dbHelper, Class<T> clazz) {
		...
		// 加载所有字段
		this.allFields = MicroTableHelper.joinFields(this.clazz.getDeclaredFields(),
		...
		L.D("clazz:" + this.clazz + " tableName:" + this.tableName
	 ...
	 * @param dbHelper 数据库操作实现类
	 */
	public MicroDBDaoImpl(SQLiteOpenHelper dbHelper) {
		this(dbHelper, null);
	}
	...
	public T queryOne(int id) {
		synchronized (lock) {
			...
			L.D("[queryOne]: select * from " + this.tableName + " where "
					+ this.idColumn + " = '" + id + "'");
	...
	public List<T> rawQuery(String sql, String[] selectionArgs,Class<T> clazz) {
		...
		try {
			lock.lock();
			checkDBOpened();
			L.D("[rawQuery]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			getListFromCursor(clazz,list, cursor);
		} catch (Exception e) {
			L.E("[rawQuery] from DB Exception.");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	...
	public boolean isExist(String sql, String[] selectionArgs) {
		...
		try {
			lock.lock();
			checkDBOpened();
			L.D("[isExist]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			if (cursor.getCount() > 0) {
				return true;
			}
		} catch (Exception e) {
			L.E("[isExist] from DB Exception.");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	...
	public List<T> queryList(String[] columns, String selection,
			...
			try {
				lock.lock();
				checkDBOpened();
				L.E("[queryList] from "+this.tableName+" where "+selection+"("+selectionArgs+")"+" group by "+groupBy+" having "+having+" order by "+orderBy+" limit "+limit);
				...
				L.E("[queryList] from DB Exception");
				e.printStackTrace();
			} finally {
				closeCursor(cursor);
				lock.unlock();
	...
	private void getListFromCursor(Class<?> clazz,List<T> list, Cursor cursor)
		...
		while (cursor.moveToNext()) {
			...
			// 加载所有字段
			List<Field> allFields = MicroTableHelper.joinFields(entity.getClass().getDeclaredFields(),
	...
	public long insert(T entity, boolean flag) {
			...
			try {
				...
				L.D("[insert]: insert into " + this.tableName + " " + sql);
				...
				for (Field relationsField : allFields) {
					...
					if(RelationsType.one2one.equals(type)){
						...
						if(relationsEntity != null){
							...
							L.D("[insert]: insert into " + relationsTableName + " " + sql);
	...
	public long[] insertList(List<T> entityList,boolean flag) {
			...
			try {
				...
				for(int i=0;i<entityList.size();i++){
					...
					L.D("[insertList]: insert into " + this.tableName + " " + sql);
					...
					if(RelationsType.one2one.equals(type)){
						...
						if(relationsEntity != null){
							...
							L.D("[insertList]: insert into " + relationsTableName + " " + sql);
							db.insert(relationsTableName, null, relationsCv);
						}
				...
				L.D("[insertList] into DB Exception.");
				e.printStackTrace();
			} finally {
				lock.unlock();
			}
	...
	public int delete(String whereClause, String[] whereArgs) {
		...
		try {
			...
			L.D("[delete]: delete from " + this.tableName + mLogSql);
	...
	public int deleteAll() {
		...
		try {
			lock.lock();
			checkDBOpened();
			L.D("[delete]: delete from " + this.tableName );
	...
	public List<Map<String, String>> queryMapList(String sql,String[] selectionArgs) {
		...
		try {
			lock.lock();
			checkDBOpened();
			L.D( "[queryMapList]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			while (cursor.moveToNext()) {
			...
			L.E( "[queryMapList] from DB exception");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	...
	public int queryCount(String sql, String[] selectionArgs) {
       ...
       try{
    	   lock.lock();
    	   checkDBOpened();
    	   L.D( "[queryCount]: " + getLogSql(sql, selectionArgs));
           cursor = db.query(this.tableName, null, sql, selectionArgs, null, null,null);
           if(cursor != null){
        	   count = cursor.getCount();
           }
       }catch (Exception e){
    	   L.E( "[queryCount] from DB exception");
           e.printStackTrace();
       }finally{
    	   closeCursor(cursor);
    	   lock.unlock();
	...
	public void execSql(String sql, Object[] selectionArgs) {
		try {
			lock.lock();
			checkDBOpened();
			L.D( "[execSql]: " + getLogSql(sql, selectionArgs));
